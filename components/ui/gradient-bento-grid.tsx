...
export interface GradientBentoGridProps {
  className?: string
  children?: React.ReactNode
}

export const GradientBentoGrid = ({ className, children }: GradientBentoGridProps) => {
  return (
    <ul
      className={cn(
...
export interface GradientBentoGridItemProps {
  area?: string
...
export const GradientBentoGridItem = ({
  area = "",
  icon,
  title,
  description,
  ...
  className,
  header,
}: GradientBentoGridItemProps) => {
  return (
    <li className={`min-h-[14rem] list-none ${area} ${className}`}>
      <HoverBorderGradient
        as="div"
        containerClassName="rounded-xl w-full h-full"
        className="dark:bg-black bg-white text-black dark:text-white w-full h-full p-0"
        duration={2}
      >
